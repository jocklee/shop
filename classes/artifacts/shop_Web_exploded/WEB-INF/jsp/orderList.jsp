...
<script>
function check(){
	var value = confirm("请确保你已经收到真实货物,是否确认收货");
	return value;
}
</script>
</head>
			...
			<table>
				<tbody>
					<s:iterator var="order" value="pageBean.list">
						<tr>
							<th colspan="6">订单编号: <s:property value="#order.oid" />
								&nbsp;&nbsp;&nbsp;&nbsp; 订单状态： <s:if test="#order.state == 1">
									...
									<a onclick="return check();" href="${pageContext.request.contextPath }/order_confirmGoods.action?oid=<s:property value="#order.oid" />&page=<s:property value="pageBean.page" />"><font color="#FF7F24">确认收货</font></a>
						...
						<s:iterator var="orderItem" value="#order.orderItems">
							<tr>
								...
								<%-- <td><a
									href="${ pageContext.request.contextPath }/order_removeCart.action?pid=<s:property value="#orderItem.product.pid"/>"
